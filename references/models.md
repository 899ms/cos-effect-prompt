...
**默认构图与光学（用户偏好）**：MJ 提示词默认带入**电影级构图与光学**——① 前景遮挡（虚化前景元素做画框式取景）；② 远 / 中 / 近三层景别结合（纵深构图）；③ 电影级构图（三分法/引导线）；④ 大光圈 f/1.4；⑤ 锐利焦点 + 奶油虚化光斑（creamy bokeh）。

```
全身照，脚下金色发光魔法阵，符文旋转，3D CGI 渲染，写实材质，体积光氛围，
前景梅花虚化做画框，梅花林为近景、女子为中景、古寺为远景，电影级三分法构图，
85mm f/1.4 大光圈，锐利对焦人物，背景奶油虚化光斑 --ar 3:4 --v 6 --stylize 200
```
